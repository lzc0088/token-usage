...
///
/// A config change (different enabled clients) is handled by **restarting** the
/// scheduler with a new `cfg`, not by mid-run mutation.
...
mod tests {
    ...
    async fn burst_of_ticks_coalesces_to_one_scan() {
        ...
        let handle = tokio::spawn(async move {
            run(mock, cfg, tick_rx, ev_tx).await;
        });
