...
<style>
  ...
  .card-slot {
    ...
    /* Default (cardOnLeft=false): the card window sits RIGHT of the
       ...
       hugs it with 12px of arrow room. The 16px-deep tail (neck 2px into
       the card) puts the tip exactly CARD_GAP past the panel edge (Rust
       CARD_GAP = +2): 2px onto the panel surface, fusing card+arrow+panel
       at a FIXED offset regardless of card width. */
